const frag = `
	...
	void main(){
		...
		vec3 redBlueNoise1 = vec3(cnoise(vec3(st.x*6. + u_time/50.,st.y*4.+ u_time/50.,u_time/2.))/1.5,
														 0.3*cnoise(vec3(st.x*6.1 + u_time/50.,st.y*4.+ u_time/50.,u_time/2.)),
														 cnoise(vec3(st.x*6.+ u_time/50.,st.y*4.+ u_time/50.,u_time/2.+5000.))/3. );
		vec3 redBlueNoise2 = vec3(cnoise(vec3(st.x*16. + u_time/80.,st.y*10.+ u_time/80.,u_time/2.))/1.5,
														 0,
														 cnoise(vec3(st.x*16.+ u_time/80.,st.y*10.+ u_time/80.,u_time/2.+5000.))/3. );
		
		vec3 redBlueNoise3 = vec3(cnoise(vec3(st.x*10. + u_time/50.,st.y*200.+ u_time/50.,u_time/2.)),
														 cnoise(vec3(st.x*10. + u_time/50.,st.y*200.+ u_time/50.,u_time/2.))*0.5,
														 cnoise(vec3(st.x*10.+ u_time/50.,st.y*200.+ u_time/50.,u_time/2.+5000.))/3. );
			//極光效果
		vec3 redBlueNoise4 = vec3(cnoise(vec3(st.x*500.+50.*cos(st.y*5.-0.5+st.x) + u_time/50.,st.y*(5.+st.x*0.01)+ u_time/30.,u_time/2.)),
														 cnoise(vec3(st.x*500.1+50.*cos(st.y*5.-0.5+st.x*1.01) + u_time/50.,st.y*(4.99+st.x*0.01)+ u_time/30.1,u_time/2.)),
														 0.6*cnoise(vec3(st.x*499.9+50.*cos(st.y*5.-0.5+st.x*1.02)+ u_time/50.,st.y*5.+ u_time/30.2,u_time/2.)));
		
		
		
		//紅色藍色噪聲
		color += canvasTexture *canvasTexture * (redBlueNoise1+redBlueNoise2*0.2)*0.45
						+ redBlueNoise3*0.4*clamp(sin(u_time/300.),0.,1.)*cos(st.y*10.+u_time/10.)
						+ (1.+canvasTexture/50.)*redBlueNoise1*4.*redBlueNoise4
													 *cnoise(vec3(st.x*12.,st.y*6.,u_time/3.))
													 *cnoise(vec3(st.x,st.y*0.1,u_time/3.))
												
													;
// 						
// 									*cos(st.y*10.+u_time/10.)*cos(st.x*3.+u_time/10.)*cnoise(vec3(u_time/50.));
